...
class ConvertFeedTest < Minitest::Test
  def test_reverse_rss
    ...
    standard = NokogiriFile.read('test/fixtures/handler/reverse_rss')
  ...
  def test_sort_rss
    ...
    Dispatcher.run(options, feed)
    out = NokogiriFile.read('output')
    standard = NokogiriFile.read('test/fixtures/handler/sort_rss')
    assert_equal out.xpath('//item').text, standard.xpath('//item').text
  end
  ...
  def test_sort_atom
    ...
    Dispatcher.run(options, feed)
    out = NokogiriFile.read('output')
    standard = NokogiriFile.read('test/fixtures/handler/sort_atom')
  ...
  def test_reverse_atom
    ...
    Dispatcher.run(options, feed)
    out = NokogiriFile.read('output')
    standard = NokogiriFile.read('test/fixtures/handler/reverse_atom')
